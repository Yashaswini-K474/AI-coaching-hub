...
// 🚨 CRITICAL: Define the token cost for this feature (e.g., 500)
const TOKEN_COST_FOR_ROOM = 500;
...
function UserInputDialog({ children, coachingOption }) {
    ...
    const router = useRouter();
    // Get user data and tokens from Context
    const { userData } = useContext(UserContext);

    // Get the Discussion Room creation mutation
    const createDiscussionRoom = useMutation(api.DiscussionRoom.CreateNewRoom);

    // ✅ NEW: Get the token consumption mutation
    ...
    const handleNext = async () => {
        ...
        // 🛑 AUTH GUARD (Your existing logic)
        if (!isConvexAuthenticated) {
            console.error("Convex Auth is not ready. Mutation aborted.");
            if (!isClerkSignedIn) {
                clerk.redirectToSignIn();
        ...
        // 🛑 TOKEN GUARD: Check if the user has enough credits
        // We use Number() because Convex credits are stored as numbers in the schema but can be complex in runtime
        if (Number(userData?.credits) < TOKEN_COST_FOR_ROOM) {
            alert(`Insufficient tokens! This action requires ${TOKEN_COST_FOR_ROOM.toLocaleString()} tokens. You have ${Number(userData?.credits).toLocaleString()}. Please upgrade your plan.`);
            return;
        }
        ...
        try {
            ...
            // 1. CRITICAL: Deduct tokens first
            await consumeTokens({ amount: TOKEN_COST_FOR_ROOM });

            // 2. Then, run the main feature logic
            const result = await createDiscussionRoom({
                topic,
                coachingOption: getCoachingOptionName(coachingOption),
                expertName: selectedExpert,
            ...
            // If both token deduction and room creation succeed:
            setOpenDialog(false);
            // The token count in Credits.jsx will automatically update via UserContext
            router.push("/discussion-room/" + result);

        } catch (error) {
            // This catches errors from both consumeTokens and createDiscussionRoom
            console.error("Operation Failed:", error);
            alert(`Failed to start session. Server error: ${error.message}`);
        } finally {
            setLoading(false);
        }
    };

    // ✅ Define the final disabled state.
    const isReadyToSubmit = isConvexAuthenticated && topic && selectedExpert;
    // Add check for null/undefined userData while loading
    const hasEnoughTokens = (Number(userData?.credits) || 0) >= TOKEN_COST_FOR_ROOM;

    const isButtonDisabled = isConvexLoading || !isReadyToSubmit || loading || !hasEnoughTokens;

    // ... rest of the component (getButtonText, isTriggerDisabled, return JSX) ...

    // The rest of the component remains the same, ensuring the button calls handleNext
    // ...
    ...
    const getButtonText = () => {
        ...
        if (!hasEnoughTokens) {
            return `Need ${TOKEN_COST_FOR_ROOM.toLocaleString()} Tokens`;
        }
        return "Next";
    }
